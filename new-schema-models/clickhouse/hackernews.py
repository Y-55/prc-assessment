...
class CharCounts(Base):
    char_ = Column(types.Int64,name='id',primary_key=True)
    count = Column(types.String,name='text')
    
    __tablename__ = 'char_counts'
    __table_args__ = (
...
CharCountsMatViewAdd = MaterializedView(
    inner_model=CharCounts,
    selectable=select(
        text("arrayJoin(ngrams(ifNull(after_text, ''), 1)) as char_"),
        text("toInt64(count(*)) as count")
    ).where(
    ...
    use_to=True,
    name=f'{CharCounts.__tablename__}_add_mv'
)

CharCountsMatViewDel = MaterializedView(
    inner_model=CharCounts,
    selectable=select(
        text("arrayJoin(ngrams(ifNull(before_text, ''), 1)) as char_"),  
        text("toInt64(count(*)) as count")  
    ).where(
    ...
    use_to=True,
    name=f'{CharCounts.__tablename__}_del_mv'
)
